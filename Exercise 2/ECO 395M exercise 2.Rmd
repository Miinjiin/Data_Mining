...
```
```{r problem 3.2.3, message=FALSE, echo=FALSE}
...
fold_cv = foreach(i=1:K, .combine='rbind') %do% {
  fold=val%>%filter(fold_id==i)
  model_cv=glm(children ~ . - arrival_date+adults:stays_in_weekend_nights + adults:total_of_special_requests + adults:average_daily_rate + average_daily_rate:total_of_special_requests, data=val_train)
...
ggplot(fold_cv_df, aes(x=fold)) +
  geom_col(aes(x=fold, y=expected_child, color='expected_child')) +
  geom_col(aes(x=fold, y=actual_child, color='actual_child')) 
...
fold_cv = foreach(i=1:K, .combine='rbind') %do% {
  train_cv=filter(fold_cv, fold_id==i)
  test_cv=filter(fold_cv, fold_id !=i)
  ...
  phat_cv=predict(model_cv, test_cv, type="response")
  actual_cv=test_cv$children
  phat_cv
  actual_cv
}
```
